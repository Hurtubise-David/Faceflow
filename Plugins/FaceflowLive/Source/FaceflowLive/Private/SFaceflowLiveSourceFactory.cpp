...
#define LOCTEXT_NAMESPACE "FaceflowLiveSourceEditor"

void SFaceflowLiveSourceFactory::Construct(const FArguments& Args)
{
	...
	[
		...
		[
			...
			[
				...
				[
					SNew(STextBlock)
					.Text(LOCTEXT("FacefloWPortNumber", "Port Number"))
				]
				+ SHorizontalBox::Slot()
				.HAlign(HAlign_Fill)
				.FillWidth(0.5f)
				[
					...
					.OnTextCommitted(this, &SFaceflowLiveSourceFactory::OnEndpointChanged)
				]
			]
			+ SVerticalBox::Slot()
			.HAlign(HAlign_Right)
			...
			[
				SNew(SButton)
				.OnClicked(this, &SFaceflowLiveSourceFactory::OnOkClicked)
				[
					SNew(STextBlock)
					.Text(LOCTEXT("Ok", "Ok"))
				]
...
void SFaceflowLiveSourceFactory::OnEndpointChanged(const FText& NewValue, ETextCommit::Type)
{
...
FReply SFaceflowLiveSourceFactory::OnOkClicked()
{
	TSharedPtr<SEditableTextBox> EditabledTextPin = EditabledText.Pin();
	if (EditabledTextPin.IsValid())
	{
