...
`;
  ...
  const maxConfig = {
      ...
      kernelFunc: ({ inputs, attrs, backend }) => {
          ...
          if (shouldExecuteOnCPU) {
              const xTexData = webglBackend.texData.get(maxInput.dataId);
              const values = xTexData.values;
              const outValues = maxImplCPU(values, tf.util.sizeFromShape(reduceShape), maxOutShape, x.dtype);
  ...
  const nonMaxSuppressionV3Config = {
      ...
      kernelFunc: ({ inputs, backend, attrs }) => {
          ...
          return tf.kernel_impls.nonMaxSuppressionV3(boxesVals, scoresVals, maxOutputSizeVal, iouThresholdVal, scoreThresholdVal);
      }
  };
